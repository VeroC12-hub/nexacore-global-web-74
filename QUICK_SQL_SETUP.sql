-- 🚀 PHASE 6: WORKFLOW AUTOMATION - QUICK SETUP
...
ON public.workflow_templates FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Admins can manage workflow templates" 
ON public.workflow_templates FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);

-- RLS Policies for workflow_instances  
CREATE POLICY "Users can view assigned workflows or created by them" 
ON public.workflow_instances FOR SELECT USING (
  assigned_to = auth.uid() OR initiated_by = auth.uid() OR
...
ON public.workflow_instances FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Assigned users and admins can update workflows" 
ON public.workflow_instances FOR UPDATE USING (
  assigned_to = auth.uid() OR initiated_by = auth.uid() OR
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'project_manager'))
);
...
ON public.workflow_steps FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Admins can manage workflow steps" 
ON public.workflow_steps FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);

-- RLS Policies for workflow_step_instances
CREATE POLICY "Users can view step instances they are involved in" 
ON public.workflow_step_instances FOR SELECT USING (
  assigned_to = auth.uid() OR completed_by = auth.uid() OR
  EXISTS (SELECT 1 FROM public.workflow_instances wi WHERE wi.id = workflow_instance_id AND (wi.assigned_to = auth.uid() OR wi.initiated_by = auth.uid())) OR
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('admin', 'project_manager'))
);

CREATE POLICY "Assigned users can update step instances" 
ON public.workflow_step_instances FOR UPDATE USING (
...
ON public.workflow_approvals FOR SELECT USING (
  approver_id = auth.uid() OR
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);

CREATE POLICY "Approvers can manage their approvals" 
ON public.workflow_approvals FOR ALL USING (
  approver_id = auth.uid() OR
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
...
ON public.workflow_automation_rules FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);

CREATE POLICY "Admins can manage automation rules" 
ON public.workflow_automation_rules FOR ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
...
-- Your Phase 6 enterprise features are now ready to use!